...
def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    for update_item in updates:
        if update_item.channel_post and ':' in update_item.channel_post.text:
            word, reply = update_item.channel_post.text.split(':', 1)
            word_reply_dict[word] = reply.strip()
    
    # Log the fetched replies
    logger.info("Fetched word-reply pairs: %s", word_reply_dict)
